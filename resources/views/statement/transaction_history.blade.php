<!DOCTYPE html>
<html>
<head>
<style>
#customers {
  font-family: Arial, Helvetica, sans-serif;
  border-collapse: collapse;
  width: 100%;
}

#customers td, #customers th {
  border: 1px solid #ddd;
  padding: 6px;
}

/* #customers tr:nth-child(even){background-color: #f2f2f2;}

#customers tr:hover {background-color: #ddd;} */

#customers th {
  padding-top: 12px;
  padding-bottom: 12px;
  text-align: left;
  /* background-color: #4CAF50; */
  color: green;
}


</style>
</head>

@php 
 
$setting = App\Models\Setting::find(1);
@endphp 
<body>
<hr>
<div class="row"> <!-- start 1st row -->
    <div style="float: left" class="col-md-4 text-center">
        <img src="data:image/png;base64,{{ base64_encode(file_get_contents($setting->logo)) }}" style="width: 320px; height: 120px;">
    </div>

    <div class="col-md-3 text-center">
        <!-- Leave this column empty for spacing -->
    </div>

    <div style="float: right" class="col-md-5 text-center">
        <p><strong style="font-size: 25px; float: center;">{{$setting->name}}</strong><br style="line-height: 2; margin-bottom: 10px;">
      Address:  {{$setting->address}} <br>
       Email: {{$setting->email}} <br>
       Contact Us: {{$setting->contact}} <br>
       
    </div>
</div> <!-- end 1st row -->



<br>
  <br>
  <br>
  <br>
  <br>
  <br>
  
  


 
 
 <hr>

      
 <table id="customers" style="font-size: 13px;">
                      
                       <thead>
                           <tr>
                           <th> <center>TRANSACTIONS HISTORY REPORT FROM: {{$startDate}} TO:   {{$endDate}} </center></th>   
                               
                           </tr>
                       </thead>
                       
                     </table>
<br>
<table id="customers" style="font-size: 13px;">
<tr style="color:green;">
      
        <th>SL</th>
        <th>Transaction Date</th>
        <th>Transaction Description</th>
        <th>Credit</th>
        <th>Debit</th>
        <th>Balance</th>
    </tr>

        
       @foreach($transactions as $key=> $item)
            <tr>
                
                <td>{{$key+1}}</td>
                <td>{{ $item->transaction_date }}</td>
                <td>{{ $item->transaction_description }}</td>
                <td>{{number_format($item->credit) }}</td>
                <td>{{number_format($item->debit) }}</td>
                <td>{{number_format($item->balance) }}</td>
               
            </tr>
            @endforeach
            <tr>
                <td></td>
                <td></td>
                <td>Grand Totals</td>
                <td>  <strong>{{$setting->currency}} {{ number_format($totalCredit) }}</strong></td>
                <td> <strong>{{$setting->currency}} {{number_format ($totalDebit)}}</strong></td>
                <td>  <strong>{{$setting->currency}} {{number_format($lastBalance) }}</strong></td>
            </tr>
        
</table>
<br>



</body>
</html>
